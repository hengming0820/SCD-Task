...
def rand_flip_SCD5(img1, img2, label1, label2, label3, label4):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
...
def rand_rot90_SCD5(img1, img2, label1, label2, label3, label4):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img1, img2, label1, label2, label3, label4
...
def rand_rot90_flip_SCD5(img1, img2, label1, label2, label3, label4):
    img1, img2, label1, label2, label3, label4 = rand_rot90_SCD5(img1, img2, label1, label2, label3, label4)
    return rand_flip_SCD5(img1, img2, label1, label2, label3, label4)
...
def random_crop_SCD(img1, img2, label1, label2, size):
    # print(img.shape)
    h = img1.shape[0]
    w = img1.shape[1]
    c_h = size[0]
...
def RandomHorizontalFlip(im1, im2, label1, label2, prob):
    im1 = Image.fromarray(im1)
    im2 = Image.fromarray(im2)
    label1=Image.fromarray(label1)
    label2=Image.fromarray(label2)

    horizontal_flip = transforms.RandomHorizontalFlip(p=prob)
    img_A = horizontal_flip(im1)
    img_B = horizontal_flip(im2)
    label_A = horizontal_flip(label1)
    label_B = horizontal_flip(label2)
...
def RandomVerticalFlip(im1, im2, label1, label2, prob):
    im1 = Image.fromarray(im1)
    im2 = Image.fromarray(im2)
    label1=Image.fromarray(label1)
    label2=Image.fromarray(label2)

    horizontal_flip = transforms.RandomVerticalFlip(p=prob)
...
def ColorJitter(im1, im2, prob):
    if prob > 0.5:
        im1 = Image.fromarray(im1)
        im2 = Image.fromarray(im2)

        horizontal_flip = transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2)
        img_A = horizontal_flip(im1)
        img_B = horizontal_flip(im2)
