...
def SCDD_eval_all(preds, labels, num_class):
    ...
    for i in range(num_class):
        true_positive = hist[i, i]
        false_positive = hist[:, i].sum() - true_positive
        false_negative = hist[i, :].sum() - true_positive
        denominator = true_positive + false_positive + false_negative
        if denominator == 0:
            class_iou[i] = 0  # 避免除以零错误
        else:
            class_iou[i] = true_positive / denominator

    kappa = cal_kappa(hist_all)
    hist_fg = hist[1:, 1:]
    c2hist = np.zeros((2, 2))
    c2hist[0][0] = hist[0][0]
    ...
    Fscd = stats.tmean([SC_Precision, SC_Recall])
    return Fscd, IoU_mean, Sek, iu[0], iu[1], kappa, class_iou, hist_all
